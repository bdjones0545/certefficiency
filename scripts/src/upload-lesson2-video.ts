/**
 * Uploads the Lesson 2 video (and a thumbnail) from attached_assets/ to GCS
 * and updates the platform_lessons DB record for Lesson 2.
 *
 * Run:  pnpm --filter @workspace/scripts run upload-lesson2
...
const VIDEO_SRC = path.join(
  REPO_ROOT,
  "attached_assets/Lesson_2__Why_Every_AI_Agent_Needs_Its_Own_Virtual_Machine_108_1785097959833.mp4",
);
const MIME_TYPE = "video/mp4";
const ORIGINAL_FILENAME = "Lesson_2__Why_Every_AI_Agent_Needs_Its_Own_Virtual_Machine.mp4";
const DURATION_SECS = 255; // confirmed via ffprobe

const THUMBNAIL_TMP = "/tmp/lesson2_thumbnail.jpg";
...
async function uploadFile(
  ...
  // Private by default — no public ACL metadata (premium lesson)
  await gcsFile.setMetadata({
    metadata: { aclPolicy: JSON.stringify({ visibility: "private", permissions: {} }) },
  });
}
...
async function main() {
  ...
  try {
    ...
    if (!updateRes.rows.length) throw new Error("Lesson 2 not found for course " + courseId);
...
Upload complete.
  ...
  AI_LESSON_2_VIDEO_ID : not needed — self-hosted
────────────────────────────────────────`);
}
